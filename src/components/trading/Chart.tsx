...
function getReserves(tokenA: string, tokenB: string, pairs: Pair[]) {
  var pair = pairs.find(
    (pair) =>
      [pair.token0, pair.token1].includes(tokenA) &&
      [pair.token0, pair.token1].includes(tokenB)
  );
  if (!pair)
    throw new Error(`No reserves found for tokens : ${[tokenA, tokenB]} `);
...
function getAmountsOut(
  ...
  for (var i = 0; i < path.length - 1; i++) {
    ...
    if (i == 0) {
      amounts[i] = amountIn.toDecimalPlaces(
        tokenIn.decimals,
        Decimal.ROUND_DOWN
      );
    }

    var [reserveIn, reserveOut]: Decimal[] = getReserves(
      tokenIn.address,
      tokenOut.address,
      pairs
    );
  ...
  return [amounts[0], amounts[amounts.length - 1]].map((amount) =>
    amount.toNumber()
  );
}
...
export default function TradingChart({
  ...
  var series = [
    {
      name: "",
      data: reservesUpdates.length
        ? reservesUpdates[0].history.map((_, index) => {
            ...
            var [inQuantity, outQuantity] = getAmountsOut(
              new Decimal(1),
              path,
              pairs,
              tokens
  ...
  var options = {
    ...
    xaxis: {
      categories: reservesUpdates.length
        ? reservesUpdates[0].history.map((update, index) =>
            formatHour(update.timestamp)
          )
        : [],
      labels: {
        show: true,
  ...
  const pathTokens = path.map((address) =>
  ...
  return (
    <Flex direction="column" padding="1.5rem" w="100%" h="100%">
      ...
      >
        <Skeleton margin={"5px"} isLoaded={isLoaded}>
          <SwitchTokens
            assetIn={assetIn}
            assetOut={assetOut}
        ...
        <Stat>
          <Skeleton margin={"5px"} isLoaded={isLoaded} minHeight="20px">
        ...
        <ButtonGroup
          ...
          colorScheme="purple"
        >
          <Button bg="purple.100" color="purple.800">
            1H
          </Button>
          <Button>1D</Button>
          <Button>1M</Button>
